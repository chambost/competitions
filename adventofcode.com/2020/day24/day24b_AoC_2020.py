...
for i in range(100) :
    for item,count in bag.copy().items() :
        if (count % 2) == 0 :
            del bag[item]
    counts = Counter()
    for coord in bag.keys() :
        for next in { "se","sw","w","nw","ne","e" } :
            c = Counter(coord+(next,))
            removeopposites(c)
            removeloops(c)
            shorten(c)
            counts[tuple(sorted(c.elements()))] += 1
    new_bag = Counter()
    for item,count in counts.items() :
        if item in bag and count in (1,2) :
            new_bag[item] += 1
        elif item not in bag and count == 2:
            new_bag[item] += 1
    bag = new_bag
