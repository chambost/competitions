...
def extract(group,enemies,boost) :
    if "(" in group :
        a,z = group.split("(")
        b,c = z.split(")")
        if ";" in b :
...
def initialise(choseninput,boost) :
    ...
    for group in first.split("\n") :
        immune_system.add( extract(group,infect_system,boost) )
    second = second.lstrip("Infection:\n")
    for group in second.split("\n") :
        infect_system.add( extract(group,immune_system,0) ) 
    return immune_system, infect_system


def solve_day24b_aoc_2018(choseninput,boost) :
    immune_system, infect_system = initialise(choseninput,boost)
    ...
    while immune_system and infect_system :
        ...
        for i,action in sorted(actions
            ...
            action()
    return sum( g.units for g in immune_system ),sum( g.units for g in infect_system )


for i in range(41) :
    print(i,solve_day24b_aoc_2018(puzzleinput,i))
